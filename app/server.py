...
def character_view_prompt(prompt: str, view: str) -> str:
...
Style: clean pixel-art sprite concept, readable silhouette, crisp hard-edged pixels, limited palette, game asset reference.
...
def action_frame_prompt(character_prompt: str, action_prompt: str, action_name: str, index: int, frame_count: int) -> str:
    return f"""
...
Frame: {index + 1} of {frame_count}; make this a distinct pose in the cycle while preserving the same character.
Use the attached front, side, and top reference images as identity references, not as edit targets.
Style: clean pixel-art sprite frame, crisp hard-edged pixels, limited palette, consistent proportions, centered full-body game sprite.
...
def save_views(character_dir: Path, prompt: str) -> dict[str, str]:
    ...
    for view in ("front", "side", "top"):
        ...
        if IMAGE_PROVIDER == "mock":
            ...
            run_codex_imagegen(path, character_view_prompt(prompt, view), generated_paths)
...
def save_action_frames(
    ...
    for idx in range(frame_count):
        ...
        if IMAGE_PROVIDER == "mock":
            ...
            run_codex_imagegen(
                path,
                action_frame_prompt(character_prompt, action_prompt, action_name, idx, frame_count),
                frame_references,
            )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_api(self, path: str) -> None:
        ...
        if self.command == "POST" and len(parts) == 6 and parts[:2] == ["api", "projects"] and parts[3] == "characters" and parts[5] == "actions":
            ...
            assets = save_action_frames(base, character.get("prompt", ""), prompt, name, frame_count, fps, reference_paths)
            data = {
                "id": action_id,
